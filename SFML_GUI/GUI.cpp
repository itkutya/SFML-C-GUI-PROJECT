...
namespace GUI
{
		...
		if (data.is_open())
		{
			...
			while (data >> type)
			{
				if (type == "BUTTON")
				{
					...
					this->m_clicked.push_back(false);
	...
	bool Button::onButtonClick(unsigned short int index)
	{
		return this->m_clicked[index];
	}
